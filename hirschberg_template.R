...
HirschbergTemplate <- function(X, Y, align, match, mismatch, gap){
  ...
  else
  {
    ...
    xmid <- floor(xlen/2) # half of the length of X
    ...
    {
      align <- HirschbergTemplate(X[(xmid+1):length(X)],DNAString(""),align,match, mismatch, gap)# call Hirschberg function for the second half of X and for an empty DNAString object
    }
    else 
    {
      align <- HirschbergTemplate(X[(xmid+1):length(X)],Y[(ymid+1):length(Y)],align,match, mismatch, gap)# call hirschberg function for the second half of X and the second part of Y
    }
  }
  return(align)
}
